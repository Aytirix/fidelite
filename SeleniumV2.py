...
class SeleniumV2():
    ...
    def handle_exception(self, e: Exception, msg: str = None) -> None:
        ...
        if msg:
            ...
            if not hasattr(self, "error_screen"):
                ...
                print_msg("ERREU", f"{date} | {msg}", 'red')
                return
            return self.error_screen(add_erreur=f"{msg} | Erreur : {exception_map.get(type(e), f'{e}')}")

    def presence_of_element(self, by: By, value: str, timeout: int = 1, find_all: bool = False, shadow: bool = False, msg: str = None, element: WebElement = None, attribut=None) -> WebElement:
        """
    ...
    def send_keys(self, by: By = None, value: str = None, keys: str = None, timeout: int = 1, msg: str = None, element: WebElement = None, clear: bool = True) -> bool:
        ...
        Paramètres :
            ...
            element (WebElement) : Un élément WebElement dans lequel insérer le texte (défaut : None).
            clear (bool) : True pour vider l'élément avant d'insérer le texte, False sinon (défaut : True).
    ...
    def ResetSession(self, refresh=True):
        """
        Cette fonction permet de réinitialiser la session du navigateur
        # param refresh : permet de rafraîchir la page si nécessaire
        """
        try:
            ...
            self.error_screen(add_erreur=f"Erreur lors de la réinitialisation de la session : {e}")
            return False
